...
(() => {
    ...
    const interval = setInterval(() => {
        let btn = null;
        if (document.getElementById('mainFrame') !== null)
            btn = document.getElementById('mainFrame').contentWindow.document.querySelector('.cke_button_file');
        else
            btn = document.querySelector('.cke_button_file');
        if (!btn)
            return;

        const entry = pluginBootstrap();
        const newBtn = document.createElement('a');
        newBtn.innerHTML = btn.innerHTML + `上传文件`;
        btn.parentNode.insertBefore(newBtn, btn);
        btn.parentNode.removeChild(btn);
        newBtn.title = "上传文件";
        newBtn.addEventListener('click', (e) => entry.click());
